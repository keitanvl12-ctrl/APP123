...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // DEBUGGING ROUTE - SIMPLE TICKETS
  app.get("/api/tickets/debug", async (req, res) => {
    try {
      console.log("🔍 Debug tickets endpoint called");
      const simpleTickets = await storage.getAllTickets();
      console.log(`🔍 Debug found ${simpleTickets.length} tickets`);
      res.json(simpleTickets);
    } catch (error) {
